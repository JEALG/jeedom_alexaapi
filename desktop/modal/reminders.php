...
//log::add('alexaapi', 'warning', '**********************************************'."http://" . config::byKey('internalAddr') . ":3456/reminders");
...
<table class="table table-condensed tablesorter" id="table_healthNetwork">
	...
	<tbody>
		...
		foreach ($json as $item) {
			...
			if ($item['status'] == 'ON') {
				$present = '<a class="btn btn-success disableReminder" data-id="' . $item['id'] . '"><span class="label label-success" style="font-size : 1em;" title="{{Actif}}"><i class="fas fa-check-circle"></i></span></a>';
			} else {
				$present = '<a class="btn btn-default enableReminder" data-id="' . $item['id'] . '"><span class="label label-default" style="font-size : 1em;" title="{{Inactif}}"><i class="fas fa-times-circle"></i></span></a>';
				$couleur = "default";
			}
			...
			echo '<td><a class="btn btn-danger deleteReminder" data-id="' . $item['id'] . '"><i class="fas fa-times-circle"></i></a></td>';
			//$present = '<span class="label label-default" style="font-size : 1em;" title="{{Inactif}}"><i class="fas fa-times-circle"></i></span>';
